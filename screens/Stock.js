...
export class Stock extends Component {
  ...
  fetchData = () => {
    ...
    fetch('http://agsgroupsindia.com/assets/ans/admin/api/product_stock.php?company_no='+ shop.shop_no, {
      method: 'GET',
      headers: {
      ...
      .then((json) => {
        if(json.Product_Stock.length > 0){
          this.setState({
            productList: json.Product_Stock
          })
        }
      })
  ...
  render() {
    ...
    if(productList.length == 0){
      return(
        ...
        >
          <Text style={{alignSelf: 'center', fontSize: 16}}>No pending stocks</Text>
        </View>
      )
    }
    return (
            ...
            <FlatList
                ...
                renderItem={({ item, index }) =>
                  <View style={styles.productlist}>
                    ...
                    <Text style={styles.prodMrp}>{item.stock}</Text>
