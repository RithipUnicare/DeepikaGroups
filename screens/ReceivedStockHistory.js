...
export class ReceivedStockHistory extends Component {
	constructor(props) {
		super(props);
	...
	componentDidMount() {
		let originalSel ='';
		let originalTo ='';
		if(this.props.route.params.p_date == undefined){
			originalSel = this.props.route.params.selDate;
			originalTo = this.props.route.params.toDate;
		} else {
			originalSel = this.props.route.params.p_date;
			originalTo = this.props.route.params.p_date;
		}
		this.setState(
			{
	...
	fetchData = () => {
		...
		fetch(
			ser_url +
				'/receivedStockReport.php?company_no=' +
				loginDetails.ID +
				'&from_date=' +
			...
			.then(json => {
				if (json.ReceivedStockList.length > 0) {
					this.setState({
						productList: json.ReceivedStockList,
					});
				...
				<View style = {{paddingBottom:'45%'}}>
					...
					{productList.length == 0 ? (
						...
						<FlatList
								...
								<View style={styles.productlist}>
									<View style={{flexDirection:'row'}}>
										<View style={{width:'15%'}}>
											...
											<View style={{width:'85%'}}>
													<Text style={styles.prodName}>{item.Godown_name}</Text>
													
													<Text style={{color: '#ffffff'}}>
															Product: {item.Product_name}
													</Text>
													<Text style={{color: '#ffffff'}}>
															Stock Received: {item.Quantity}
													</Text>
							...
							keyExtractor={(item, index) => 'SharedStock' + index}
						/>
					)}
			</View>
...
export default connect(mapStateToProps, {addProductList})(ReceivedStockHistory);
