...
export class GodownStock extends Component {
				...
				.then((json) => {
					...
					if(json.categoryProduct.length > 0){
						this.setState({
							...
							sortedData : json.categoryProduct.sort((a, b) => a.millilitre - b.millilitre)
						})
	...
	renderSection = (title, keyName) => (

		<View style={styles.productlist}>
			<Text style={styles.prodName}>{title}</Text>
			{this.state.sortedData.map((item, index) => (
			    <Text key={`${title}-${index}`} style={styles.rowText}>
					{item.millilitre} ML: {item[keyName].toFixed(2)}
				</Text>
			))}
			{/* Calculate the total sum */}
			<Text style={[styles.rowText, styles.totalText]}>
		    	Total {title}: {this.state.sortedData.reduce((sum, item) => sum + Number(item[keyName]), 0).toFixed(2)}
		  	</Text>
		</View>
  	);
				...
				<View style={{paddingBottom:'45%'}}>
					...
					{productList.length == 0 ? (
						...
						<ScrollView contentContainerStyle={styles.content}>
						    <>
						        {this.renderSection("Opening Stock", "opening_Stock")}
						        {this.renderSection("KSBCL", "KSBCL")}
						        {this.renderSection("Total", "total")}
						        {this.renderSection("Sales", "sales")}
						        {this.renderSection("Closing Stock", "closing_Stock")}
						    </>
						    {details === 'Litre' ? (
						        <>
						            {this.renderSection("Opening Stock (L)", "opening_Stock_litre")}
						            {this.renderSection("KSBCL (L)", "KSBCL_litre")}
						            {this.renderSection("Total (L)", "total_litre")}
						            {this.renderSection("Sales (L)", "sales_litre")}
						            {this.renderSection("Closing Stock (L)", "closing_Stock_litre")}
						        </>
						    ):null}
						</ScrollView>
					)}
