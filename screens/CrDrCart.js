...
export class CrDrCart extends Component {
  ...
  fetchData = () => {
    ...
    fetch(
      ser_url +
      '/get_payment_cart.php?company_id=' + 
      loginDetails.ID +
      '&payment_date=' +
  ...
  rmFromCart = (prData) => {
    ...
    fetch(
      ser_url +
      '/delete_payment_cart.php?company_id=' + 
      loginDetails.ID +
      '&payment_id=' + 
      prData.payment_id  +
      '&payment_date=' + 
  ...
  placeOrder = () => {
    ...
    if (!selDate) {
      ...
      fetch(
        ser_url +
        '/add_final_payment.php?company_id=' + 
        loginDetails.ID +
        '&payment_date=' +
        ...
        .then((json) => {
          ...
          if(json.Success == 0){
            ...
            ToastAndroid.showWithGravity(
              "Final Payment amount submitted successfully.",
              ToastAndroid.SHORT,
              ToastAndroid.TOP
            );
                ...
                <View style={styles.productlist}>
                  <View style={{flexDirection:'row'}}>
                      ...
                      <View style={{width:'90%'}}>
                        ...
                        <Text style={styles.prodMrp}>{'\u20B9'}{item.payment_amount}</Text>
                        {/* <Text style={styles.prodRetail}>Total Price - {'\u20B9'}{(item.c_price * item.qty).toFixed(2)}</Text> */}
                        <Text style={styles.prodRetail}>{item.payment_type}</Text>
                        <View style={{flexDirection: 'row', marginVertical: 5,}}>
                          ...
                          Alert.alert(
                            "Remove Item",
                            "Do you want to remove payment item?",
                            [
                              {
                                text: "Yes",
...
export default connect(mapStateToProps, {addProductList})(CrDrCart);