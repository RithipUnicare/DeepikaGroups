...
export class ShareStockDateList extends Component {
	constructor(props) {
		super(props);
	...
	fetchData = () => {
		...
		fetch(
			ser_url +
				'/shareStockDateList.php?company_no=' +
				loginDetails.ID +
				'&from_date=' +
			...
			.then(json => {
				if (json.SharedStockList.length > 0) {
					this.setState({ productList: json.SharedStockList });
					this.props.addProductList(json.SharedStockList);
				} else {
					this.setState({ productList: [] });
										...
										navigate('ShareStockHistory', {
											...
											<View style={{width:'90%'}}>
												<Text style={styles.prod}>
													<Text style={styles.prodName}>Shared From: </Text>
...
export default connect(mapStateToProps, {addProductList})(ShareStockDateList);
