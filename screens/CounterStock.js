...
export class CounterStock extends Component {
	...
	fetchData = () => {
		...
		if(Moment(selDate).format('yyyy-MM-DD') > Moment(toDate).format('yyyy-MM-DD')){
			...
			if(loginDetails.Type == 'Godown'){
				url = ser_url +
					'/counter_stock_history.php?c_id=' +
					shop.counter_id +
					'&g_id=' +
					loginDetails.ID +
					'&type=' +
			...
			.then(json => {
					// console.log(json);
				if (json.Counter_stock.length > 0) {
					this.setState({
						productList: json.Counter_stock,
					});
					this.props.addProductList(json.Counter_stock);
				} else {
					this.setState({
						productList: [],
					});
	...
	getTotalAmount = (prodListData) => {
		...
		prodListData.map((item) => {
			tamt = tamt + (parseFloat(item.purchase_amount) *
												...
												parseFloat(item.counter_received)) -
								...
								<View style={styles.productlist}>
									<View style={{flexDirection:'row'}}>
										...
										<View style={{width:'90%'}}>
											...
											<Text style={{color: '#ffffff'}}>
												Received: {parseFloat(item.counter_received)}
											</Text>
											<Text style={{color: '#ffffff', fontWeight:'bold'}}>
												Total: {parseFloat(item.opening_stock)+parseFloat(item.counter_received)}
											</Text>
											<Text style={{color: '#ffffff'}}>
											...
											<Text style={{color: '#ffffff'}}>
												Total Amount: {'\u20B9'} {
													...
													((parseFloat(item.opening_stock) +
													parseFloat(item.counter_received)) -
													(parseFloat(item.sales)))).toFixed(4)
												}
											</Text> 
										</View>
									</View> 
						...
						{productList.length > 0 ? (
							<View style={{bottom: 0, flexDirection: 'column',position:'absolute',width:'100%'}}>
								<Text style={{fontSize: 18, textAlign: 'center', color: '#ffffff'}}>
									Total Amount - {'\u20B9'}
									{this.getTotalAmount(productList).toFixed(4)}
								</Text>
								</View> 
						): null}
...
export default connect(mapStateToProps, {addProductList})(CounterStock);