...
export class ExpenseHistory extends Component {
	...
	componentDidMount() {
		let originalSel ='';
		let originalTo ='';
		if(this.props.route.params.e_date == undefined){
			originalSel = this.props.route.params.selDate;
			originalTo = this.props.route.params.toDate;
		} else {
			originalSel = this.props.route.params.e_date;
			originalTo = this.props.route.params.e_date;
		}
		this.setState(
			{
								...
								<View style={{flexDirection:'row'}}>
										...
										<View style={{width:'90%'}}>
											<View style={{flexDirection: 'row', alignItems: 'center'}}>
												...
												<Text style={styles.prodMrp}>{item.title}</Text>
											</View>
											<View style={{flexDirection: 'row', alignItems: 'center'}}>
				...
				{productList.length > 0 ? (
					<View style={{bottom: 0, flexDirection: 'column',position:'absolute',width:'100%'}}>
						<Text style={{fontSize: 18, textAlign: 'center', color: '#ffffff'}}>
							Total Amount - {'\u20B9'}
							{this.getTotalAmount(productList)}
						</Text>
					</View>
				) : null}
...
export default connect(mapStateToProps, {addProductList})(ExpenseHistory);
