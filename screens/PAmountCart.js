...
export class PendingCart extends Component {
	...
	fetchData = () => {
			...
			loginDetails.ID, 
		{
			...
			.then((json) => {
				...
				if(json.PAmount_list.length > 0){
					let tamt = 0;
					json.PAmount_list.map((item) => {
						tamt = tamt + parseFloat(item.amount);
					});
					this.setState({
						productList: json.PAmount_list,
						totalAmt: tamt
					})
					this.props.addProductList(json.PAmount_list);
				}else{
					this.setState({
						productList: [],
						totalAmt: 0
	...
	rmFromCart = (prData) => {
		...
		fetch( 
			ser_url +
			'/delete_pAmount_cart.php?pAmount_id=' + 
			prData.pAmount_id +
			'&c_no=' + 
			loginDetails.ID, 
		{
		...
		if (!selDate) {
				...
				Moment(selDate).format('yyyy-MM-DD'), 
			{
				...
				.then((json) => {
					...
					if(json.Success == 0){
						ToastAndroid.showWithGravity(
							"Final partner amount submitted successfully.",
							ToastAndroid.SHORT,
							ToastAndroid.TOP
						);
	...
	render() {
		...
		if(productList.length == 0){
			return(
				...
				>
					<Text style={{alignSelf: 'center', fontSize: 16, color: '#ffffff'}}>No Partner Amount Added Yet</Text>
				</View>
			)
		}
		return (
								...
								<View style={styles.productlist}>
									<View style={{flexDirection:'row'}}>
										...
										<View style={{width:'90%'}}>
											...
											{/* <Text style={styles.prodRetail}>ShareHolder - {item.shareholder_name}</Text>
											<Text style={styles.prodRetail}>Code - {item.Product_Code}</Text> */}
											{/* <Text style={styles.prodMrp}>Qty - {item.qty}</Text> */}
											<Text style={styles.prodRetail}>Amount - {'\u20B9'}{item.amount}</Text>
