...
export class PendingCart extends Component {
	...
	fetchData = () => {
		...
		fetch(
			ser_url +
			'/get_pending_cart.php?company_no=' + 
			loginDetails.ID +
			'&status=unpaid', 
		{
			method: 'GET',
			...
			.then((json) => {
				...
				if(json.Pending_list.length > 0){
					let tamt = 0;
					json.Pending_list.map((item) => {
						tamt = tamt + parseFloat(item.pending_amount);
					});
					this.setState({
						productList: json.Pending_list,
						totalAmt: tamt
					})
					this.props.addProductList(json.Pending_list);
				}else{
					this.setState({
						productList: [],
						totalAmt: 0
	...
	rmFromCart = (prData) => {
		...
		// alert(ser_url+'/counter_transfer_delete.php?cart_id='+ prData.cart_id +'&company_no='+ loginDetails.ID +'&type='+ loginDetails.Type)
		fetch( 
			ser_url +
			'/delete_pending_cart.php?pending_id=' + 
			prData.pending_id +
			'&company_no=' + 
			loginDetails.ID +
			'&type=' + 
	...
	placeOrder = () => {
		...
		if (!selDate) {
			...
			fetch(
				ser_url +
				'/add_pending_final.php?company_no=' + 
				loginDetails.ID +
				'&pending_date=' +
				Moment(selDate).format('yyyy-MM-DD'), 
			{
				...
				.then((json) => {
					...
					if(json.Success == 0){
						ToastAndroid.showWithGravity(
							"Final pending amount submitted successfully.",
							ToastAndroid.SHORT,
							ToastAndroid.TOP
						);
	...
	handleSearch = text => {
		...
		let filteredData = this.props.rproductList.filter(function (item) {
			return item.reason.toLowerCase().includes(formattedQuery);
		});
	...
	render() {
		...
		if(productList.length == 0){
			return(
				...
				>
					<Text style={{alignSelf: 'center', fontSize: 16, color: '#ffffff'}}>No Pending Amount Added Yet</Text>
				</View>
			)
		}
		return (
				<View style={styles.container}>
								...
								<View style={styles.productlist}>
									<View style={{flexDirection:'row'}}>
										...
										<View style={{width:'90%'}}>
											...
											<Text style={styles.prodRetail}>Selected Price - {'\u20B9'}{item.pending_amount}</Text>
					...
					{productList.length > 0 ? <View style={{bottom: 0, flexDirection: 'column'}}><Text style={{fontSize: 18, textAlign: 'center', color: '#ffffff'}}>Total Amount - {'\u20B9'}{totalAmt.toFixed(2)}</Text><TouchableOpacity style={{backgroundColor: 'green', justifyContent: 'center', alignItems: 'center', padding: 15,borderRadius: 5,}} onPress={() => Alert.alert(
						"Final Submit",
						"Are you sure you want to submit final pending amount?",
						[
							{
								text: "Cancel",
								style: "cancel"
...
export default connect(mapStateToProps, {addProductList})(PendingCart);