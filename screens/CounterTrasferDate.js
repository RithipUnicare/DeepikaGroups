...
export class PurchaseList extends Component {
	...
	fetchData = () => {
		...
		this.setState({
			...
			fetch(
				ser_url +
					'/counterTransfer_date_report.php?company_no=' +
					loginDetails.ID +
					'&from_date=' +
			...
			.then(json => {
				// console.log(json);
				if (json.CounterTransfer_List.length > 0) {
					this.setState({
						productList: json.CounterTransfer_List,
					});
				} else {
					this.setState({
						productList: [],
				...
				<View style={{paddingBottom:'45%'}}>
					...
					{productList.length == 0 ? (
						...
						<FlatList
							...
							renderItem={({item, index}) => (*/
								<View style={styles.productlist}>
									<TouchableOpacity 
										...
										navigate('CounterTransferHistory', {
											c_date:item.Date,
										})
									}>
											<View style={{width:'30%'}}>
							...
							keyExtractor={(item, index) => 'CounterTransferHistory' + index}
						/>
					)}
