...
export class ReceivedStockDateList extends Component {
	constructor(props) {
		super(props);
		...
		this.state = {
			...
			isLoading: true,
			modalVisible: false,
			selDate: firstDayOfMonth, // Set first day of current month
			pickerVisible: false,
			toDate: today, // Set today's date
	...
	fetchData = () => {
		...
		fetch(
			ser_url +
				'/receivedStockDateList.php?company_no=' +
				loginDetails.ID +
				'&from_date=' +
			...
			.then(json => {
				if (json.ReceivedStockList.length > 0) {
					this.setState({ productList: json.ReceivedStockList });
					this.props.addProductList(json.ReceivedStockList);
				} else {
					this.setState({ productList: [] });
				...
				<View style = {{paddingBottom:'45%'}}>
					...
					{productList.length == 0 ? (
						...
						<FlatList
										...
										navigate('ReceivedStockHistory', {
											...
											<View style={{width:'90%'}}>
												<Text style={styles.prod}>
													<Text style={styles.prodName}>Received From: </Text>
												...
												<Text style={styles.prod}>
													...
													<Text style={styles.prodMrp}>{item.Date}</Text>
												</Text>
							...
							keyExtractor={(item, index) => 'ShareStock' + index}
						/>
					)}
			</View>
...
export default connect(mapStateToProps, {addProductList})(ReceivedStockDateList);
