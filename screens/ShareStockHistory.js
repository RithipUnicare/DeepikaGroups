...
export class ShareStockHistory extends Component {
	constructor(props) {
		super(props);
	...
	fetchData = () => {
		...
		fetch(
			ser_url +
				'/sharedStockReport.php?company_no=' +
				loginDetails.ID +
				'&from_date=' +
			...
			.then(json => {
				if (json.SharedStockList.length > 0) {
					this.setState({
						productList: json.SharedStockList,
					});
...
export default connect(mapStateToProps, {addProductList})(ShareStockHistory);
